...
//		Expressions expr = new Expressions();
		...
		short password = 32766;
//		short password = -32760;
		char[] cipher = new char[clearText.length];
		for (int i = 0; i < clearText.length; i++) {
			cipher[i] = (char) (clearText[i] + password);
		}
		char[] clearText2 = new char[clearText.length];
		for (int i = 0; i < clearText.length; i++) {
			clearText2[i] = (char) (cipher[i] - password);
		}
		System.out.println(Arrays.equals(clearText, clearText2));
	}
}
